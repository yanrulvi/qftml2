...
from data.preprocess import (
    split_and_shuffle_data, preprocess_pipeline,
    normalize_labels_for_regression, denormalize_predictions
)
...
def process_window(
    ...
    if config.is_regression:
        model.eval()
        X_valid_t = torch.FloatTensor(X_valid_pca)
        with torch.no_grad():
        ...
        if y_min is not None and y_max is not None:
            ...
            print(f'    [DEBUG] Denorm preds: min={preds_denorm.min():.4f}, max={preds_denorm.max():.4f}, mean={preds_denorm.mean():.4f}')
            print(f'    [DEBUG] Denorm targets: min={targets_denorm.min():.4f}, max={targets_denorm.max():.4f}, mean={targets_denorm.mean():.4f}')
...
def run_experiment(config: ExperimentConfig, exp_dir: str):
    ...
    for r in results_by_window:
        print(f'{r["Tmax"]:8.1f}  {r["best_valid_acc"]:10.4f}  {r["n_features"]:8d}')
    ...
    if config.mode == ExperimentMode.BOUNDARY_SENSING:
        ...
        ax.axvline(x=light_time, color='red', linestyle='--', alpha=0.5,
                   label=f'Light crossing time ≈ {light_time:.0f}')
        ax.legend()
        ax.set_title('Boundary Sensing — Accuracy vs Measurement Time (Fig. 2)')
    elif config.mode == ExperimentMode.THERMOMETRY:
        heisenberg_time = 1.0 / config.physics.wD
        ax.axvline(x=heisenberg_time, color='red', linestyle='--', alpha=0.5,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(exp_dir, 'plots', 'accuracy_vs_time.pdf'), dpi=150)
    plt.close()
    
    df_summary = pd.DataFrame({'Tmax': T_list, 'best_valid_acc': acc_list})
    df_summary.to_csv(os.path.join(exp_dir, 'accuracy_vs_time.csv'), index=False)
