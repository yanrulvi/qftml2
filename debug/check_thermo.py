...
for k, case in enumerate(config.cases):
    sigma_0 = evolution.lattice.get_initial_covariance(
        evolution.H_thermal[k], temperature=case.temperature
    )
    ...
    print(
        f"  T={case.temperature:.6f}: tr(σ)={trace_sigma:.2f}, log|σ|={logdet:.4f}"
    )
...
print(
    f"  Отношение сигнал/шум: {(q_det_values.max()-q_det_values.min())/noise_amplitude:.2f}"
)
