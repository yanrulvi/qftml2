...
try:
    from physics.evolution import (
        mult_omega, build_projectors,
        compute_unitaries_stepped, evolve_projectors
    )
...
def test_probe_initialization():
    ...
    if is_identity:
        ...
        print(f"  ⚠️ sigma_0 отличается от I, макс. недиаг. элемент: {off_diag:.2e}")
...
def test_projectors_structure():
    ...
    if np.allclose(P1, expected_P1, atol=1e-10):
        ...
        print(f"  P1[{q_idx},{p_idx}] = {P1[q_idx, p_idx]:.6f} (ожидание -0.5)")
        print(f"  P1[{p_idx},{q_idx}] = {P1[p_idx, q_idx]:.6f} (ожидание -0.5)")
        all_ok = False
...
def test_no_evolution():
    ...
    # Эволюция при t=0
    U_list, U_adj_list = compute_unitaries_stepped(F, N_times=1, Tmin=0.0, Tmax=0.0)
...
def test_boundary_difference():
    ...
    if abs(F1[0, 1]) > 1e-10 and abs(F1[1, 0]) > 1e-10 and \
       abs(F2[0, 1]) < 1e-10 and abs(F2[1, 0]) < 1e-10:
    ...
    for B in [1, 2, 3]:
        ...
        sigma_0 = lattice.get_initial_covariance(
            F_th, temperature=0.0,
            is_signal=is_signal,
            Gsignal=norm['Gsignal']
        )
        ...
        trajectories[B] = traj
        print(f"  B={B}: ⟨q⟩={traj[0]:.8f}, ⟨r⟩={traj[1]:.8f}, ⟨p⟩={traj[2]:.8f}")
    
    # Проверяем различимость
    diff_12 = np.linalg.norm(np.array(trajectories[1]) - np.array(trajectories[2]))
    diff_13 = np.linalg.norm(np.array(trajectories[1]) - np.array(trajectories[3]))
    diff_23 = np.linalg.norm(np.array(trajectories[2]) - np.array(trajectories[3]))
    ...
    print(f"  sigma_0[0,0] для B=3 = {sigma_0[0, 0]:.6f} (должно быть 1/g = {1/norm['Gsignal']:.6f})")
    
    if diff_12 > 1e-8:
        print("  ✅ B1 и B2 различимы")
    else:
        print("  ⚠️ B1 и B2 почти неразличимы — это ожидаемо для данного времени")
