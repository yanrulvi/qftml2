...
def preprocess_pipeline(
    ...
    print(f'PCA: {X_train.shape[1]} → {n_components} компонент '
          f'(сохранено {100 * eigenvalues[:n_components].sum() / eigenvalues.sum():.1f}% дисперсии)')
    
    # Применяем PCA к train и valid
    X_train_pca = apply_pca(X_train, X_mean, eigenvectors, eigenvalues, n_components)
    X_valid_pca = apply_pca(X_valid, X_mean, eigenvectors, eigenvalues, n_components)
