...
def create_boundary_config() -> ExperimentConfig:
    ...
    config.physics = PhysicsConfig(
        sigma=4.2, HbarCbySig=7.14, Ksig=7.0,
        LatticeLength=10, mcc=0.1, wD=10.0, lam=10.0,
        Gsignal=3.1548,
    )
    
    # Измерения: от 2 до 22 с шагом 1 (20 окон)
    config.measurement = MeasurementConfig(
        dt=1.0, Tmin=2.0, Tmax=22.0,
        N_times=10, N_tom=1e6,  # 10^6 для теста, 10^22 для статьи
    )
    
    # Обучение
    config.ml = MLConfig(
        N_samples=500, n_epochs=500,
        minibatch_size=100, learning_rate=0.01, L2_reg=0.001,
    )
...
def create_thermometry_config() -> ExperimentConfig:
    ...
    config.physics = PhysicsConfig(
        sigma=4.2, HbarCbySig=7.14, Ksig=7.0,
        LatticeLength=10, mcc=0.1, wD=10.0, lam=10.0,
        TMean=TMean, TDev=0.1*TMean,
    )
    
    config.measurement = MeasurementConfig(
        dt=0.02, Tmin=0.02, Tmax=0.10,
        N_times=10, 
        N_tom=1e10,  # УВЕЛИЧЕНО
    )
    ...
    L = config.physics.LatticeLength
    config.cases = []
    for i, frac in enumerate(np.arange(0.90, 1.11, 0.05)):
        T = frac * TMean
        config.cases.append(CaseLabel(
            name=f'{int(frac*100)}-{int((frac+0.05)*100)}%',
            abv=f'{frac:.2f}',
            prob=1.0, y=i,
            boundary_type=1,
            distance=L // 2,
