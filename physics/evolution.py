...
def apply_tomography_noise(
    ...
    if N_tom <= 10:
        ...
        for i in range(len(trajectory)):
            result[i] = (med_trajectory[i] + trajectory[i]) * chi2_sample / (N_tom - 1) - med_trajectory[i]
    else:
        # Гауссово приближение по ЦПТ
        for i in range(len(trajectory)):
            noise = np.random.randn() * np.sqrt(2.0 / (N_tom - 1))
            result[i] = trajectory[i] + (trajectory[i] + med_trajectory[i]) * noise
...
class FieldEvolution:
    ...
    def compute_projectors_for_window(
        ...
        for k, F in enumerate(self.H_dynamic):
            U_list, U_adj_list = compute_unitaries_stepped(F, N_times, Tmin, Tmax)
    ...
    def generate_all_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for t_idx in range(1, len(windows)):
            ...
            print(f'  Window {t_idx}/{len(windows)-1}: Tmin={Tmin:.3f}, Tmax={Tmax:.3f}')
